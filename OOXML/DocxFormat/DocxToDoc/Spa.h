...
namespace Docx2Doc
{
	...
	public:
		...
		inline void Update ()
		{
			...
			DocFileFormat::FormatUtils::SetBytes((bytes + offset), m_lid);				offset += sizeof(m_lid);
			...
			if (m_bFlip) // FLIP
			{
				...
				DocFileFormat::FormatUtils::SetBytes((bytes + offset), dX - dHeight);	offset += sizeof(int);
				DocFileFormat::FormatUtils::SetBytes((bytes + offset), dY - dWidth);	offset += sizeof(int);
				DocFileFormat::FormatUtils::SetBytes((bytes + offset), dX + dHeight);	offset += sizeof(int);
				DocFileFormat::FormatUtils::SetBytes((bytes + offset), dY + dWidth);	offset += sizeof(int);
			}
			else
			{
				DocFileFormat::FormatUtils::SetBytes((bytes + offset), m_rca.left);	offset += sizeof(m_rca.left);
				DocFileFormat::FormatUtils::SetBytes((bytes + offset), m_rca.top);		offset += sizeof(m_rca.top);
				DocFileFormat::FormatUtils::SetBytes((bytes + offset), m_rca.right);	offset += sizeof(m_rca.right);
				DocFileFormat::FormatUtils::SetBytes((bytes + offset), m_rca.bottom);	offset += sizeof(m_rca.bottom);
			}
	...
	public:
		...
		int	m_bFlip;		//	если объект повернут по определенному углу то нужно флипнуть RCA структуру
	};
}
