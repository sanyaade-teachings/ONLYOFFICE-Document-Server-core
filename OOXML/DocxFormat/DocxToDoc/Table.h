...
namespace Docx2Doc
{
	class Table: public ITextItem
	{
		struct TableRowWithOffset
		{
			...
			TableRowWithOffset() : tableRow(), tableRowOffset(0)
			{
			}

			TableRowWithOffset( const TableRow& _tableRow, unsigned int _tableRowOffset ) : tableRow(_tableRow), tableRowOffset(_tableRowOffset)
			{
			}
		};
