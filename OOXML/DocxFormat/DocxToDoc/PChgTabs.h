...
namespace Docx2Doc
{
	...
	public:
		...
		explicit PChgTabsAdd( const std::vector<XAS>& _rgdxaAdd, const std::vector<TBD>& _rgtbdAdd ) : bytes(NULL), sizeInBytes(0)
		{
			unsigned int cTabs = std::min( _rgdxaAdd.size(), _rgtbdAdd.size() );
			...
			if ( this->bytes != NULL )
			{
				...
				{
					DocFileFormat::FormatUtils::SetBytes( ( this->bytes + offset ), (short)_rgdxaAdd[i] );
					offset += sizeof(XAS);
				}
	...
	public:
		...
		explicit PChgTabsDel( const std::vector<XAS>& _rgdxaDel ) : bytes(NULL), sizeInBytes(0)
		{
			...
			if ( this->bytes != NULL )
			{
				...
				for ( unsigned int i = 0; i < cTabs; i++ )
				{
					DocFileFormat::FormatUtils::SetBytes( ( this->bytes + offset ), (short)_rgdxaDel[i] );
					offset += sizeof(XAS);
				}
			}
		}
	...
	public:
		...
		explicit PChgTabsDelClose( const std::vector<short>& _rgdxaDel, const std::vector<XAS_plusOne>& _rgdxaClose ) : bytes(NULL), sizeInBytes(0)
		{
			unsigned int cTabs = std::min( _rgdxaDel.size(), _rgdxaClose.size() );
			...
			if ( this->bytes != NULL )
			{
				...
				for ( unsigned int i = 0; i < cTabs; i++ )
				{
					DocFileFormat::FormatUtils::SetBytes( ( this->bytes + offset ), _rgdxaDel[i] );
					offset += sizeof(short);
				}

				for ( unsigned int i = 0; i < cTabs; i++ )
				{
					DocFileFormat::FormatUtils::SetBytes( ( this->bytes + offset ), (short)_rgdxaClose[i] );
