...
namespace Docx2Doc
{
	...
	public:
		...
		{
			std::vector<Prl> chpxProperties = this->lPUpxChpx.GetProperties();
			std::vector<Prl> papxProperties = this->lPUpxPapx.GetProperties();

			std::vector<Prl> properties;

			for ( std::vector<Prl>::const_iterator iter = chpxProperties.begin(); iter != chpxProperties.end(); iter++ )
			{
				properties.push_back( *iter );  
			}

			for ( std::vector<Prl>::const_iterator iter = papxProperties.begin(); iter != papxProperties.end(); iter++ )
			{
				properties.push_back( *iter );  
			}
	...
	public:
		StkTableGRLPUPX():
		  ...
		  const std::vector<Prl> GetProperties() const
		  {
			  std::vector<Prl> chpxProperties = this->lPUpxChpx.GetProperties();
			  std::vector<Prl> papxProperties = this->lPUpxPapx.GetProperties();
			  std::vector<Prl> tapxProperties = this->lPUpxTapx.GetProperties();
			  ...
			  for ( std::vector<Prl>::const_iterator iter = tapxProperties.begin(); iter != tapxProperties.end(); iter++ )
			  {
				  properties.push_back( *iter );  
			  }
	...
	public:
		...
		explicit GrLPUpxSw( const Constants::StyleType _styleType, const Docx2Doc::LPUpxPapx& _lPUpxPapx, const Docx2Doc::LPUpxChpx& _lPUpxChpx, const Docx2Doc::LPUpxTapx& _lPUpxTapx ) : bytes(NULL), sizeInBytes(0), sizeInBytesWithoutPadding(0)
		{
			switch ( _styleType )
			{
			case Constants::styleTypeParagraph:
				{
					*this = GrLPUpxSw( Docx2Doc::StkParaGRLPUPX( _lPUpxPapx, _lPUpxChpx ) );
				}
				break;

			case Constants::styleTypeCharacter:
				{
					*this = GrLPUpxSw( Docx2Doc::StkCharGRLPUPX( _lPUpxChpx ) );
				}
				break;

			case Constants::styleTypeTable:
				{
					*this = GrLPUpxSw( Docx2Doc::StkTableGRLPUPX( _lPUpxTapx, _lPUpxPapx, _lPUpxChpx ) );
				}
				break;

			case Constants::styleTypeNumbering:
				{
					*this = GrLPUpxSw( Docx2Doc::StkListGRLPUPX( _lPUpxPapx ) );
				}
				break;
			}
		}
