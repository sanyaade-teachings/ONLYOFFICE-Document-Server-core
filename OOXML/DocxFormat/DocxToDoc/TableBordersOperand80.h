...
namespace Docx2Doc
{
	...
	public:
		...
		bytes(NULL), cb(0)
		{
			...
			if ( bytes != NULL )
			{
				brcTop					=	Brc80MayBeNil( DocFileFormat::FormatUtils::BytesToUInt32( bytes, ( 0 + sizeof(unsigned short) + sizeof(BYTE) ), prlSize ) );
				brcLeft					=	Brc80MayBeNil( DocFileFormat::FormatUtils::BytesToUInt32( bytes, ( 4 + sizeof(unsigned short) + sizeof(BYTE) ), prlSize ) );
				brcBottom				=	Brc80MayBeNil( DocFileFormat::FormatUtils::BytesToUInt32( bytes, ( 8 + sizeof(unsigned short) + sizeof(BYTE) ), prlSize ) );
				brcRight				=	Brc80MayBeNil( DocFileFormat::FormatUtils::BytesToUInt32( bytes, ( 12 + sizeof(unsigned short) + sizeof(BYTE) ), prlSize ) );
				brcHorizontalInside		=	Brc80MayBeNil( DocFileFormat::FormatUtils::BytesToUInt32( bytes, ( 16 + sizeof(unsigned short) + sizeof(BYTE) ), prlSize ) );
				brcVerticalInside		=	Brc80MayBeNil( DocFileFormat::FormatUtils::BytesToUInt32( bytes, ( 20 + sizeof(unsigned short) + sizeof(BYTE) ), prlSize ) );
